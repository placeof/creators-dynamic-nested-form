...
export function SubCreators({
  ...
  const { append, remove, fields } = useFieldArray({
    name: `categories.${categoryIndex}.creators.${creatorIndex}.subCreators`,
    control,
  })
  return (
    <div className="mt-1">
      {fields.map((subCreator, subCreatorIndex) => {
        return (
          <div
            className="px-6 bg-gray-300 shadow-lg rounded-lg mt-2"
            key={subCreator.id}
          >
            ...
            <input
              ...
              {...register(
                `categories.${categoryIndex}.creators.${creatorIndex}.subCreators.${subCreatorIndex}.name`
              )}
            />
            <div className="text-red-600">
              {
                errors.categories?.[categoryIndex]?.creators?.[subCreatorIndex]
                  ?.subCreators?.[subCreatorIndex]?.name?.message
              }
            </div>
          </div>
        )
